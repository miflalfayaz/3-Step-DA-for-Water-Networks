...
wdsfile         = "Input_Files/MOD.inp"                         # WDS Model Directory
bdemfile        = "Input_Files/MOD_base_demands.txt"            # Base Demands
topofile        = "Input_Files/MOD_topology.txt"                # Topology file
assetfile       = "Input_Files/MOD_assets.txt"                  # Assets file
lnkidfile       = ""
nodidfile       = ""
...
no              = 4                                 #  number of sources

sim_time        = 24                                 #  Simualtion time (hours)
ne              = 30                                 #  Ensemble size, number of simulations per tstep
Dmult           = 1.2                               #  General multiplier to all demands


# Select the Head Observations
H_observ        = np.arange(1,13)                   # select the observation nodes HEAD
#H_observ = np.random.randint(0, nn, sH)            # random selection of the observation nodes for Head
#H_observ        = np.arange(1,269)
...
#Q_observ        = np.arange(1, 318)                # select the observation nodes HEAD
...
RzQ_v           = 2                                 # variance of FLOWS measurement errors

# Select the Demand Observations
q_observ        = np.array([  3,  28,  39,  41,
                      56, 113, 115, 117,
                      128, 158, 169])               # select the observation nodes for Demand
#q_observ = np.random.randint(0, nn, sq)            # Random Selection of observation nodes for Demand
#q_observ        = np.arange(1, 269)                # select the observation nodes HEAD
vzq_v           = 0.0                               # mean of the Demand measurement errors
Rzq_v           = 0.2                               # variance of Demand measurement errors

# Output file directory
Res_fname = "DA_{}_E{}_T{}.data".format(wdsfile[-7:-3], ne, sim_time)
...
for ne in [5, 10, 15, 20, 25, 30, 40, 50, 100]:
    ...
    Res_fname = "RESULTS/DA_{}_E{}_T{}.data".format(wdsfile[-7:-3], ne, sim_time)           
...
for RzH_v in [0.001, 0.1, 0.2, 0.3, 0.5, 1]:
    for RzQ_v in [0.001, 1, 2, 3, 5, 10]:
        for Rzq_v in [0.001, 0.1, 0.2, 0.3, 0.5, 1]:
            ...
            my_file = Path("RESULTS/DA_{}_RH{}_RQ{}_Rq{}.data".format(wdsfile[-7:-3], RzH_v, RzQ_v, Rzq_v))
            if my_file.is_file():
                print('DA_{}_RH{}_RQ{}_Rq{}.data exists'.format(wdsfile[-7:-3], RzH_v, RzQ_v, Rzq_v))
...
for vzH in [0.001, 0.1, 0.2, 0.3, 0.5, 1]:
    ...
    for vzH_v in vzH_normal:
        my_file = Path("RESULTS/DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH, vzQ_v, vzq_v, i))
        if my_file.is_file():
            print("DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH, vzQ_v, vzq_v, i))
            # file exists
            files.append(my_file)
            count = len(files)
            ...
            print('Run:', i)
            print("DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH, vzQ_v, vzq_v, i))
            Res_fname = "RESULTS/DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH, vzQ_v, vzq_v, i)
            Run = Py_DA_WD.Three_Step_EnKF(wdsfile, bdemfile, topofile, assetfile, lnkidfile, nodidfile, 
                              npi, nn, no, sim_time, ne, Dmult, Res_fname, 
                              H_observ, vzH_v, RzH_v,
                              Q_observ, vzQ_v, RzQ_v,
...
for vzQ in [0.001, 1, 2, 3, 5, 10]:
    ...
    for vzQ_v in vzQ_normal:
        my_file = Path("RESULTS/vz/vzQ/DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH_v, vzQ, vzq_v, i))
        if my_file.is_file():
            print("DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH_v, vzQ, vzq_v, i))
            # file exists
            files.append(my_file)
            count = len(files)
            print("Completed {} / 600".format(count)) 
            ...
            print('Run:', i)
            print("DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH_v, vzQ, vzq_v, i))
            Res_fname = "RESULTS/vz/Fiv/vzQ/DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH_v, vzQ, vzq_v, i)
            Run = Py_DA_WD.Three_Step_EnKF(wdsfile, bdemfile, topofile, assetfile, lnkidfile, nodidfile, 
                              npi, nn, no, sim_time, ne, Dmult, Res_fname, 
                              H_observ, vzH_v, RzH_v,
                              Q_observ, vzQ_v, RzQ_v,
...
for vzq in [0.001, 0.1, 0.2, 0.3, 0.5, 1]:
    ...
    i = 0 
    for vzq_v in vzq_normal:
        my_file = Path("RESULTS/vz/vzqd/DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH_v, vzQ_v, vzq, i))
        if my_file.is_file():
            print("DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH_v, vzQ_v, vzq, i))
            # file exists
            files.append(my_file)
            count = len(files)
        ...
        print('Run:', i)
        print("DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH_v, vzQ_v, vzq, i))
        Res_fname = "RESULTS/vz/Fiv/vzqd/DA_{}_E{}_vzH{:.2f}_vzQ{:.2f}_vzq{:.2f}_{}.data".format(wdsfile[-7:-3], ne, vzH_v, vzQ_v, vzq, i)
        Run = Py_DA_WD.Three_Step_EnKF(wdsfile, bdemfile, topofile, assetfile, lnkidfile, nodidfile, 
                          npi, nn, no, sim_time, ne, Dmult, Res_fname, 
                          H_observ, vzH_v, RzH_v,
                          Q_observ, vzQ_v, RzQ_v,
...
# Previously optimised HEad Sensors
# H_net = [246,  58, 140,  57, 239, 260, 207, 252, 
#           253, 180, 109, 165, 123,  39, 204, 126, 
#           191, 145, 198, 220, 203, 168,  44,  92, 
#           225, 195,  14, 194,  71,  54, 116,  22, 
#           193,  70, 209, 144,  91,  74,  82,  25]
...
# Previously optimised Demand Sensors
# q_net = [161, 63,  95, 205, 128,  59,  31, 100, 
#           162, 62, 227, 219,  39, 136, 184,  10, 
#           156,  9, 159, 133, 179,  65, 163,  35, 
#           114, 25,   8, 209, 188,   2, 176, 158, 
#           155, 138, 32,  33, 187,  86,   7, 218]
...
# Previously optimised Flow Sensors
# Q_net = [ 10, 100, 299, 130, 137, 135, 138, 307, 
#           217,  65, 146, 201, 288,  23, 222, 169, 
#           129, 210, 237, 105, 192, 223, 189,  67, 
#           33,  98, 276, 244, 161, 219, 145, 267, 
#           248,  84, 243,  80, 160,  96,  52,  19]
...
for sH in range(1, H_sensors + 1):
    try:
        with open("RESULTS/DA_{}_E{}_sH{}_test.data".format(wdsfile[-7:-3], ne, sH), 'rb') as filehandle:
        ...
        for H_sens in srch_spac:
            ...
            Res_fname = "RESULTS/DA_{}_E{}_sH{}_test.data".format(wdsfile[-7:-3], ne, sH)
            Sim_Res = Py_DA_WD.Three_Step_EnKF(wdsfile, bdemfile, topofile, assetfile, lnkidfile, nodidfile, 
                              npi, nn, no, sim_time, ne, Dmult, Res_fname, 
                              H_observ, vzH_v, RzH_v,
                              Q_observ, vzQ_v, RzQ_v,
...
for sq in range(1, q_sensors + 1):
    try:
        with open("RESULTS/DA_{}_E{}_sq{}_test.data".format(wdsfile[-7:-3], ne, sq), 'rb') as filehandle:
            # Read the data as a binary data stream
            Sim_Res = pickle.load(filehandle)
            q_net = Sim_Res[1][0]
            print('The file exists and has been read.')
        ...
        for q_sens in srch_spac:
            ...
            Res_fname = "RESULTS/DA_{}_E{}_sq{}_test.data".format(wdsfile[-7:-3], ne, sq)
            Sim_Res = Py_DA_WD.Three_Step_EnKF(wdsfile, bdemfile, topofile, assetfile, lnkidfile, nodidfile, 
                              npi, nn, no, sim_time, ne, Dmult, Res_fname, 
                              H_observ, vzH_v, RzH_v,
                              Q_observ, vzQ_v, RzQ_v,
...
for sQ in range(1, Q_sensors + 1):
    try:
        with open("RESULTS/DA_{}_E{}_sQf{}_test.data".format(wdsfile[-7:-3], ne, sQ), 'rb') as filehandle:
            # Read the data as a binary data stream
            Sim_Res = pickle.load(filehandle)
            Q_net = Sim_Res[2][0]
            print('The file exists and has been read.')
    ...
    #    head_net_temp = []
        ...
        for Q_sens in srch_spac:
            ...
            Q_net_temp.append(Q_sens-1)
            Q_observ = np.array(Q_net_temp)
            Res_fname = "RESULTS/DA_{}_E{}_sQf{}_test.data".format(wdsfile[-7:-3], ne, sQ)
            Sim_Res = Py_DA_WD.Three_Step_EnKF(wdsfile, bdemfile, topofile, assetfile, lnkidfile, nodidfile, 
                              npi, nn, no, sim_time, ne, Dmult, Res_fname, 
                              H_observ, vzH_v, RzH_v,
                              Q_observ, vzQ_v, RzQ_v,
...
Res_fname = "RESULTS/GA_{}_sH{}_sq{}_sQ{}_TT{}.data".format(wdsfile[-7:-3], H_sensors, q_sensors, Q_sensors, timestr)
...
for Khaze in HWC_Range:
    ...
    Res_fname = "RESULTS/DA_{}_E{}_T{}_K{}.data".format(wdsfile[-7:-3], ne, sim_time, Khaze)
    Sim_Res = Py_DA_WD.Three_Step_EnKF(wdsfile, bdemfile, topofile, assetfile, lnkidfile, nodidfile, 
                      npi, nn, no, sim_time, ne, Dmult, Res_fname, 
                      H_observ, vzH_v, RzH_v,
                      Q_observ, vzQ_v, RzQ_v,
