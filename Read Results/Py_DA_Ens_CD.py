# -*- coding: utf-8 -*-
"""
Created on Mon Mar 20 00:10:47 2023
...
for ne in [5, 10 , 15, 20, 25, 30, 40, 50, 100]:
    ...
    with open('RESULTS/CD/IMP/'+resfilename, 'rb') as filehandle:
        # Read the data as a binary data stream
        Sim_Res = pickle.load(filehandle)
...
fig, ax1 = plt.subplots(figsize=(8, 5.5),constrained_layout=True, dpi=300)
...
ax1.plot(x, y3,'.--', markersize=10, label = 'Modena Before')
#ax1.plot(x, y5,'.:', markersize=10, label = 'Modena After')
ax1.plot(x, y4,'.--', markersize=10, label = 'Five Reservoir Before Fix')
#ax1.plot(x, y6,'.:', markersize=10, label = 'Five Reservoir After Fix')
...
for i in range(0, len(x)):
 ...
 #   ax1.annotate('{:.0f}'.format(y4[i]), [x[i]+0.5,y4[i]-100])
    ax1.annotate('{:.0f}'.format(y5[i]), [x[i]+0.5,y5[i]-30])
  #  ax1.annotate('{:.0f}'.format(y6[i]), [x[i]+0.5,y6[i]-100])
#%%
...
y4 = y2/x
#%%
fig, ax2 = plt.subplots(figsize=(10, 5.5),constrained_layout=True, dpi=300)
...
ax2.legend(fontsize=12);  # Add a legend
for i in range(len(x)):
    ax2.annotate('{:.1f}'.format(y3[i]), [x[i]+1,y3[i]+1])
    ax2.annotate('{:.1f}'.format(y4[i]), [x[i]+1,y4[i]+1])
